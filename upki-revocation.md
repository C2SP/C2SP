...
How the cache is populated (for example production, hosting, downloading,
verification, and incremental update of CRLite clubcard data) is out of scope
for this document.
...
The **serial number** of a certificate is the DER-encoded contents (that is, the
...
A clubcard filter is a two-stage filter based on solving systems of linear
equations over `GF(2)`. The construction, its parameters, and the query
algorithm are specified in [Schanck 2025][] and this document does not restate
them. Where the on-disk structures defined here carry values that parameterize
a clubcard query, they are described as inputs to the algorithm in that paper.
...
- The layout of a cache directory containing one or more clubcard filter files.
...
Filter versions 0 through 3, and the index version "upkiidx0" predate this
specification and are not covered here. An implementation of this specification
MAY reject filters whose version identifier is not 4, and index files with
a version identifier that is not "upkiidx1".
...
format](#filter-file-format). Their names MUST be between 1 and 32 bytes
long and MUST consist only of the ASCII characters `A`–`Z`, `a`–`z`,
`0`–`9`, hyphen (`-`), period (`.`), and underscore (`_`). The names `.`
and `..` MUST NOT be used. Beyond those restrictions, filenames are opaque
to this specification.

There MAY be additional metadata files in the cache directory that describe when
the filters were fetched, their expected checksums, etc. These details are
outside of the scope of this specification and are not required for efficient
revocation checking.
...
- **Clubcard query width** `W`. We select `W = 4`. Each query is expressed as
  a linear equation over `W` distinct 64-bit lanes (totalling 256 bits).

## Filter file format

A single filter file within the revocation cache directory is a self-contained
...
to signal the presence of additional metadata (for example describing
a partition of the filter) that is omitted in version 4.
...
block's exception list, and each entry is a variable-length certificate serial
...
Each `name` slot holds a filter file's name, padded with `0x00` bytes to 32
bytes total. There is no explicit length field; the actual name is the byte
prefix up to (but not including) the first `0x00`, or all 32 bytes if none
is present. An implementation MUST treat an index file containing a name
that violates the restrictions in [Revocation cache directory
layout](#revocation-cache-directory-layout) as malformed.
...
Together, these invariants ensure that any `index.bin` reader may observe
...
3. If opening a filter file identified in step 1 of the [Revocation check
   procedure](#revocation-check-procedure) fails because the file does not
   exist, the reader MUST treat this as evidence that the `index.bin` it is
   using has been superseded by a newer one that no longer references the
   missing filter. The reader SHOULD retry the check from the beginning,
   reopening `index.bin` at least once before returning an error. An
   implementation MAY bound the number of retries to avoid unbounded restart
   under pathological update patterns.
...
2. Locate the `IssuerEntry` in the filter's `ClubcardIndex` whose `issuer`
   ...
   **not enrolled**: the filter covers a relevant timestamp window but was
   ...
   precedes the issuer lookup, **not enrolled** is only reported by a filter
   that covers at least one of the certificate's SCTs.
...
the two MAY collapse them, treating them as a **not covered** result. Like **not
...
implementation MAY return **revoked** as soon as any filter yields it,
without querying the remaining covering filters. No other outcome supports
early termination. After a filter yields **not revoked**, **not enrolled**,
or **not covered**, the remaining covering filters MUST still be queried,
since any of them may yet yield **revoked**.
...
(for example, minimum number of SCTs, SCT diversity) and does not require that
the logs named by the SCTs be logs the caller trusts. Callers with specific
...
for pathological input (for example, nested variable-length fields, `filter_idx`
values into the filename table, arbitrary `offset` values in the log directory)
and each MUST be checked for safe operation. Filenames read from the index are
used to open files relative to the cache directory. The character restrictions
on filter file names exclude path separators and MUST be enforced before a name
is used in a filesystem operation, preventing an index file from directing
reads outside the cache directory.
