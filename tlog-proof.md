...
This document describes a textual format containing an offline-verifiable proof
that some data has been logged to a transparency log and that its inclusion has
been observed by some number of witnesses.
...
More specifically, a proof composes the following information:
  ...
  - an optional hint, with application-specific meaning.

```
c2sp.org/tlog-proof
index 73894
hint YWdlLXYxLjIuMS1kYXJ3aW4tYXJtNjQudGFyLmd6
...
referred to as *spicy signatures*.
...
The first line MUST be the string `c2sp.org/tlog-proof`.

The second line MUST start with the string `index` followed by a space (U+0020)
...
The third line MAY start with the string `hint` followed by a space (U+0020) and
a base64-encoded opaque hint. If no hint is present, this line is omitted.
Appications MUST NOT implicitly trust the hint, as it is not authenticated. (Use
cases for the hint include out-of-band context, or additional data necessary to
reconstruct the record hash, such as a VRF proof.)

The next zero or more non-empty lines are the base64-encoded Merkle inclusion
proof for the entry at the specified index in the checkpointed tree, one hash
per line, starting from the leaf's sibling hash up to the root's child hash. See
[RFC 6962, Section 2.1.1] for the precise format.
