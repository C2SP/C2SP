...
The file body is encrypted with the [agead](./agead.md) streaming encryption algorithm, with `k` set to the file key, and `nonce` set to the above nonce.

The file MUST NOT be modified without re-encrypting it as a new file with a
fresh nonce.
