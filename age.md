...
SealBase function from [RFC 9180][] with the following parameters:
...
fixed string `p256tag`, the second is the base64-encoded encapsulated key *enc*
from SealBase, and the third is the base64-encoded tag.
...
the HPKE SealBase function from [RFC 9180][] with the following parameters:

  * KEM: QSF-P256-MLKEM768-SHAKE256-SHA3256 from [draft-ietf-hpke-pq-01][]
  * KDF: HKDF-SHA256
  * AEAD: ChaCha20Poly1305
...
It is then encoded as a recipient stanza with three arguments: the first is the
fixed string `p256mlkem768tag`, the second is the base64-encoded encapsulated
key *enc* from SealBase, and the third is the base64-encoded tag.
