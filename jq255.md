...
## Notations
  ...
  - A *group element* is defined as a pair `{ P, P+N }` for any curve
    ...
    *representants* of the group element. The group law is denoted
    ...
    since `N` has order 2, it does not matter which representant is used
    for each group element. The encoding process ensures that the same
    output is obtained for both representants of the same group element.

  - Integers modulo `r` are called *scalars*. Since `r` is prime, the
    scalars constitute a finite field. A group element can be multiplied
    by a scalar by using any integer representant of the scalar.
...
The pseudocode assumes that the following functions are available:
  ...
  - `half(x)` divides the field element `x` by two. It is usually
    ...
    of the constant `(q+1)/2` if the dropped bit was non-zero. It is
    considered to be a "fast" operation (i.e. at least as fast as an
  ...
  - `is_square(x)` return `True` if `x` is a square in the finite field,
    or `False` otherwise.
...
to also use signatures as commiments on a unique message: these
